...
(function () {
  ...
  function getFrequencyLabel(freq) {
    switch (freq) {
      case 'WEEK_1_3': return '1° y 3° semana del mes';
      case 'WEEK_2_4': return '2° y 4° semana del mes';
      case 'WEEK_1': return 'Solo 1° semana del mes';
      case 'WEEK_2': return 'Solo 2° semana del mes';
      case 'WEEK_3': return 'Solo 3° semana del mes';
      case 'WEEK_4': return 'Solo 4° semana del mes';
  ...
  function ensureDateInitialized(dateStr) {
    ...
    if (dayOfWeek >= 1 && dayOfWeek <= 5) {
      ...
      const template = (state.weeklyTemplate || DEFAULT_WEEKLY_TEMPLATE)[templateIndex];
      if (template) {
        template.forEach((item, idx) => {
          let doctor = null;
          if (item.doctorId) {
            doctor = state.doctors.find(d => d.id === item.doctorId);
          }
          if (!doctor && item.docSearch) {
            doctor = state.doctors.find(d => d.name.toUpperCase().includes(item.docSearch.toUpperCase()));
          }
          // Do not initialize assignment if doctor was deleted from the clinic!
          if (!doctor) return;
  ...
  function initData() {
    ...
    if (currentVersion !== DATA_VERSION) {
      ...
      if (savedRooms && savedDoctors && savedAssignments) {
        ...
        state.weeklyTemplate = savedTemplate ? JSON.parse(savedTemplate) : JSON.parse(JSON.stringify(DEFAULT_WEEKLY_TEMPLATE));
  ...
  function saveData() {
    ...
    localStorage.setItem(STORAGE_KEYS.TEMPLATE, JSON.stringify(state.weeklyTemplate));
  ...
  function setupEventListeners() {
    ...
    // Robust Modal Close delegation & Backdrop click
    document.addEventListener('click', (e) => {
      const closeBtn = e.target.closest('[data-close]') || e.target.closest('.modal-close');
      if (closeBtn) {
      ...
      } else if (e.target.classList && e.target.classList.contains('modal-backdrop')) {
        ...
        visibleShifts.forEach(shiftKey => {
          ...
          if (assignments.length > 0) {
            assignments.forEach(assignment => {
              ...
              const isActiveThisWeek = isDoctorActiveThisWeek(state.selectedDate, assignment.frequency);
              ...
              if (isActiveThisWeek) {
                  ...
                  <div class="slot-item slot-busy">
                    <div class="doctor-assigned-card">
                      ...
                      <div class="doc-details">
                        ...
                        ${assignment.frequency && assignment.frequency !== 'ALL' ? `<div style="font-size:0.72rem; color:var(--giovo-blue); font-weight:600;"><i class="fa-solid fa-repeat"></i> ${getFrequencyLabel(assignment.frequency)}</div>` : ''}
                ...
                slotsHtml += `
                  <div class="slot-item slot-periodic-inactive" title="${docName} tiene este espacio asignado (${getFrequencyLabel(assignment.frequency)}), pero NO atiende en esta semana puntual.">
                    <div class="doctor-assigned-card">
                      <div class="doc-avatar" style="background-color: #94a3b8; color: #fff;">${docInitial}</div>
                      <div class="doc-details">
                        <div class="doc-name" style="color: #475569;">${docName}</div>
                        <div class="doc-spec" style="color: #64748b;">${docSpec}</div>
                        <div class="doc-hours"><i class="fa-regular fa-clock"></i> ${shiftInfo.label} (${assignment.startTime} - ${assignment.endTime})</div>
                        <div><span class="badge-periodic-off"><i class="fa-solid fa-calendar-minus"></i> No atiende esta semana</span></div>
                        <div style="font-size:0.72rem; color:#64748b; font-style:italic; margin-top:2px;">${getFrequencyLabel(assignment.frequency)}</div>
                        ${assignment.notes ? `<div class="doc-notes-tag"><i class="fa-solid fa-comment-medical"></i> ${assignment.notes}</div>` : ''}
                      </div>
                    </div>
                    <div class="slot-actions">
                      ...
                      <button class="btn btn-emerald btn-xs" onclick="MediApp.openAssignModal('${room.id}', '${shiftKey}', null, '${state.selectedDate}')" title="Asignar cobertura o reemplazo">
                        <i class="fa-solid fa-plus"></i> Cubrir
                      </button>
                    </div>
                  </div>
                `;
  ...
  function openAssignModal(roomId, shiftKey = 'MANANA', assignmentId = null, targetDate = null) {
    ...
    // Calculate day label for recurring weekly checkbox
    const parts = dateToSet.split('-');
    const dateObj = new Date(parts[0], parts[1] - 1, parts[2]);
    const dayNames = ['Domingos', 'Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábados'];
    const dayLabel = dayNames[dateObj.getDay()] || 'este día';
    const dayLblEl = document.getElementById('assignDayOfWeekLabel');
    if (dayLblEl) dayLblEl.textContent = `todos los ${dayLabel}`;

    const freqEl = document.getElementById('assignFrequency');
    const repeatEl = document.getElementById('assignRepeatWeekly');
    ...
    if (assignmentId) {
      ...
      if (assignment) {
        ...
        if (freqEl) freqEl.value = assignment.frequency || 'ALL';
        if (repeatEl) repeatEl.checked = false; // When editing single entry, default unchecked unless chosen
      }
    } else {
      ...
      if (repeatEl) repeatEl.checked = true; // New assignments default to recurring
  ...
  function handleAssignSubmit(e) {
    ...
    const frequency = document.getElementById('assignFrequency')?.value || 'ALL';
    const repeatWeekly = document.getElementById('assignRepeatWeekly')?.checked;
    ...
    // Apply change across all weeks if requested
    if (repeatWeekly) {
      const parts = targetDate.split('-');
      const dateObj = new Date(parts[0], parts[1] - 1, parts[2]);
      const dayOfWeek = dateObj.getDay(); // 1=Mon .. 5=Fri
      if (dayOfWeek >= 1 && dayOfWeek <= 5) {
        const tIdx = dayOfWeek - 1;
        if (!state.weeklyTemplate) {
          state.weeklyTemplate = JSON.parse(JSON.stringify(DEFAULT_WEEKLY_TEMPLATE));
        }
        if (!state.weeklyTemplate[tIdx]) state.weeklyTemplate[tIdx] = [];

        // Remove previous template entry for same room & shift & doctor
        const docObj = state.doctors.find(d => d.id === doctorId);
        state.weeklyTemplate[tIdx] = state.weeklyTemplate[tIdx].filter(item => 
          !(item.roomId === roomId && item.shift === shiftKey && item.doctorId === doctorId)
        );

        // Add updated entry to template
        state.weeklyTemplate[tIdx].push({
          roomId: roomId,
          shift: shiftKey,
          doctorId: doctorId,
          docSearch: docObj ? docObj.name : '',
          startTime: startTime,
          endTime: endTime,
          frequency: frequency,
          notes: notes
        ...
        // Also update matching assignments in all initialized dates on that weekday
        state.assignments.forEach(a => {
          if (a.date === targetDate) return;
          const aParts = a.date.split('-');
          const aDate = new Date(aParts[0], aParts[1] - 1, aParts[2]);
          if (aDate.getDay() === dayOfWeek && a.roomId === roomId && a.shift === shiftKey && a.doctorId === doctorId) {
  ...
  // Pending removal state for confirm modal
  let pendingRemovalAssignment = null;

  function removeAssignment(id) {
    const assignment = state.assignments.find(a => a.id === id);
    if (!assignment) return;

    pendingRemovalAssignment = assignment;
    const doc = state.doctors.find(d => d.id === assignment.doctorId);
    const room = state.rooms.find(r => r.id === assignment.roomId);
    const docName = doc ? doc.name : 'Médico';
    const roomName = room ? room.name : 'Consultorio';

    const msgEl = document.getElementById('confirmRemoveMessage');
    if (msgEl) {
      msgEl.innerHTML = `¿Cómo desea eliminar la asignación de <strong>${docName}</strong> en <strong>${roomName}</strong> (${assignment.startTime} - ${assignment.endTime} hs)?`;
    }
    openModal('modalConfirmRemove');
  }

  function handleConfirmRemoveAllWeeks() {
    if (!pendingRemovalAssignment) return;
    const { doctorId, roomId, shift, date } = pendingRemovalAssignment;
    const parts = date.split('-');
    const dateObj = new Date(parts[0], parts[1] - 1, parts[2]);
    const dayOfWeek = dateObj.getDay();

    // 1. Remove from all dates matching this weekday, room, shift, and doctor
    state.assignments = state.assignments.filter(a => {
      const aParts = a.date.split('-');
      const aDate = new Date(aParts[0], aParts[1] - 1, aParts[2]);
      if (aDate.getDay() === dayOfWeek && a.roomId === roomId && a.shift === shift && a.doctorId === doctorId) {
        return false;
      }
      return true;
    });

    // 2. Remove from weekly template
    if (dayOfWeek >= 1 && dayOfWeek <= 5 && state.weeklyTemplate) {
      const tIdx = dayOfWeek - 1;
      if (state.weeklyTemplate[tIdx]) {
        state.weeklyTemplate[tIdx] = state.weeklyTemplate[tIdx].filter(item => {
          if (item.roomId === roomId && item.shift === shift) {
            if (item.doctorId === doctorId) return false;
            const doc = state.doctors.find(d => d.id === doctorId);
            if (doc && item.docSearch && doc.name.toUpperCase().includes(item.docSearch.toUpperCase())) return false;
          }
          return true;
        });
      }
    ...
    pendingRemovalAssignment = null;
    showToast('Turno eliminado permanentemente de todas las semanas', 'warning');
  }

  function handleConfirmRemoveOnlyDate() {
    if (!pendingRemovalAssignment) return;
    state.assignments = state.assignments.filter(a => a.id !== pendingRemovalAssignment.id);
    saveData();
    renderAll();
    closeModal('modalConfirmRemove');
    pendingRemovalAssignment = null;
    showToast('Turno liberado únicamente en esta fecha puntual', 'info');
  }
  ...
  function deleteDoctor(docId) {
    ...
    const docName = doc ? doc.name : 'este profesional';
    if (confirm(`¿Está seguro de eliminar a ${docName}? Se eliminará por completo de todas las agendas, turnos asignados y plantillas.`)) {
      // 1. Remove from doctors list
      state.doctors = state.doctors.filter(d => d.id !== docId);

      // 2. Remove all assignments across ALL dates permanently
      state.assignments = state.assignments.filter(a => a.doctorId !== docId);

      // 3. Remove from weeklyTemplate
      if (state.weeklyTemplate && Array.isArray(state.weeklyTemplate)) {
        state.weeklyTemplate = state.weeklyTemplate.map(dayArr => {
          if (!Array.isArray(dayArr)) return dayArr;
          return dayArr.filter(item => {
            if (item.doctorId === docId) return false;
            if (doc && item.docSearch && doc.name.toUpperCase().includes(item.docSearch.toUpperCase())) return false;
            return true;
          });
        });
      ...
      renderDoctors();
      renderAll();
      showToast(`Profesional ${docName} eliminado por completo de la agenda`, 'warning');
    }
  }
